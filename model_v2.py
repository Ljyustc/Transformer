...
class ModelM(nn.Module):
    def __init__(self, tokenizer, init_from=('gpt2', 'gpt2', 'gpt2'), neuron_dim=100, num_iterations=5):
        ...
        # Initialize modules
        if init_from[0].startswith('gpt2'):
            self.encoder = GPT2Model.from_pretrained(init_from[0])  # Encoder: GPT-2 (Frozen Parameters)
        else:
            self.encoder = GPT2Model(GPT2Config())
        if init_from[1].startswith('gpt2'):
            self.think_model = GPT2Model.from_pretrained(init_from[1])  # Thinking Module: GPT-2 (Trainable Parameters)
        else:
            self.think_model = GPT2Model(GPT2Config())
        if init_from[2].startswith('gpt2'):
    ...
    def forward(self, input_ids, attention_mask, targets=None):
        ...
        # Save initial values of the first 50 columns
        fixed_columns = neuron_matrix[:, :, :50]  # (batch, 100, 50)
        ...
        for i in range(self.num_iterations):
            ...
            # Pass to Model
            # print(thinking_input.size())
            # print(updated_attention_mask.size())
            output = self.think_model(inputs_embeds=thinking_input, attention_mask=updated_attention_mask).last_hidden_state  # (batch, seq_len + 100, dim)
            ...
            if targets != None:
                logits = self.decode(neuron_matrix, targets[i])
