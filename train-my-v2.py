...
# I/O
out_dir = 'out'
checkpoint_name = f"ckpt_{time.strftime('%Y%m%d_%H%M%S')}_v1_ddp_1650K.pt"
...
def compute_loss(model, batch):
    ...
    for t in targets:
        print("targets:", (t!=tokenizer.eos_token_id).sum(dim=1))
...
train_dataset = ProblemAnswerDataset('/data/jyliu/transformer_project/pretrain_data/1650k-Train.jsonl', tokenizer)
...
train_loader = DataLoader(train_dataset, batch_size=batch_size, sampler=train_sampler, collate_fn=CollateFn(tokenizer.eos_token_id), num_workers=4, pin_memory=True)
val_loader = DataLoader(val_dataset, batch_size=batch_size, sampler=val_sampler, collate_fn=CollateFn(tokenizer.eos_token_id), num_workers=4, pin_memory=True)
